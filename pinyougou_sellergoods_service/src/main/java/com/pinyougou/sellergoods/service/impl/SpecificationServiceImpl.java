...
public class SpecificationServiceImpl implements SpecificationService {
	...
	public void update(Specification specification){
		...
		TbSpecificationOption specOption=new TbSpecificationOption();
		specOption.setSpecId(id);
		tbSpecificationOptionMapper.delete(specOption);
		if(specification.getSpecificationOptionList()!=null){
			for (TbSpecificationOption option : specification.getSpecificationOptionList()) {
				option.setSpecId(specification.getSpecification().getId());
				tbSpecificationOptionMapper.insertSelective(option);
	...
	public Specification findOne(Long id){
		...
		result.setSpecification(specificationMapper.selectByPrimaryKey(id));
		Example example=new Example(TbSpecificationOption.class);
		Example.Criteria criteria = example.createCriteria();
		criteria.andEqualTo("specId",id);
		result.setSpecificationOptionList(tbSpecificationOptionMapper.selectByExample(example));
		return result;
	}
	...
	public void delete(Long[] ids) {
		...
		tbSpecificationOptionMapper.deleteByExample(example2);
	}
